...
public extension ExtWrapper where Base: UIView  {
    ...
    static func priority(high highView: UIView, low lowView: UIView, for axis: NSLayoutConstraint.Axis = .horizontal) {
        lowView.setContentHuggingPriority(.defaultLow, for: axis)
        lowView.setContentCompressionResistancePriority(.defaultLow, for: axis)
        highView.setContentHuggingPriority(.defaultHigh, for: axis)
...
public extension ExtWrapper where Base: UIView {
    ...
    func isVisible(fully: Bool = false, edgeInsets: UIEdgeInsets = .zero) -> Bool {
        ...
        //Ext.inner.ext.log("rect: \(rect) | \(base.window?.frame ?? .zero) | \(base.superview?.frame ?? .zero)")
        ...
        Ext.inner.ext.log("fully: \(fully) | visibleRect: \(visibleRect) | rect: \(rect) | intersectionRect: \(rect.intersection(screenRect))")
...
public extension ExtWrapper where Base: UIView {
    ...
    func uiImage(opaque: Bool = false, scale: CGFloat = UIScreen.main.scale) -> UIImage? {
        ...
        return render.image { context in
            //base.drawHierarchy(in: base.bounds, afterScreenUpdates: true) // 该方法会存在截取 UIView 快照失败的情况
...
public extension ExtWrapper where Base: UIView {
    ...
    func subtrackMaskView(_ view: UIView, fillBackground: Bool = false) {
        ...
        //Ext.inner.ext.log("sutrack target: \(targetFrame) | backgroundColor: \(String(describing: backgroundColor))")
        ...
        if backgroundColor == nil, fillBackground {
            //Ext.inner.ext.log("view is alpha & fillBackground")
            view.backgroundColor = .white
            view.layer.render(in: context)
            view.backgroundColor = nil
        } else {
            //Ext.inner.ext.log("view is not alpha")
            view.layer.render(in: context)
        }
        
        let image = UIGraphicsGetImageFromCurrentImageContext()?.ext.subtractMaskImage
        //Ext.inner.ext.log("view backgroundColor ")
...
private extension ExtWrapper where Base: UIImage {
    ...
    var subtractMaskImage: UIImage? {
        ...
        //Ext.inner.ext.log("draw bitmap...| scale: \(scale) | pixel: \(pixelWidth) \(pixelHeight) | pixel \(cgImage.width) \(cgImage.height) | \(bitmapBytesPerRow) ")
        ...
        if let data = context.data {
            ...
            //Ext.inner.ext.log("data: \(data) | type: \(data) | pixelBuffer: \(pixelBuffer)")
            ...
            //Ext.inner.ext.log("date end | pixelBuffer [0, 0]: \(pixelBuffer[0]) | [x: y] [\(bitmapBytesPerRow - 1), \(pixelHeight - 1)]")
        }
        Ext.inner.ext.log("subtract mask image duration: \(Date().timeIntervalSince(startTime))")
